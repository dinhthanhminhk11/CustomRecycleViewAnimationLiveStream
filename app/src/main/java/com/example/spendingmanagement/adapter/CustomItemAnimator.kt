...
open class CustomItemAnimator : SimpleItemAnimator() {
    private var sDefaultInterpolator: OvershootInterpolator? = null
    ...
    override fun runPendingAnimations() {
        ...
        if (changesPending) {
            ...
            if (removalsPending) {
                val itemView = changes[0].oldHolder?.itemView
                if (itemView != null) ViewCompat.postOnAnimationDelayed(
                    itemView,
                    changer,
                    removeDuration
                )
    ...
    override fun animateMove(
        ...
        val view = holder.itemView
        val translateX = fromX + holder.itemView.translationX.toInt()
        val translateY = fromY + holder.itemView.translationY.toInt()
        resetAnimation(holder)
        val deltaX = toX - translateX
        val deltaY = toY - translateY
        ...
        mPendingMoves.add(MoveInfo(holder, translateX, translateY, toX, toY))
        return true
    }
        ...
        infoList.indices.reversed()
            ...
            .filter {
                endChangeAnimationIfNecessary(
                    it,
                    item
                ) && it.oldHolder == null && it.newHolder == null
            }
    ...
    private fun resetAnimation(holder: RecyclerView.ViewHolder) {
        if (sDefaultInterpolator == null) {
            sDefaultInterpolator = OvershootInterpolator()
        }
